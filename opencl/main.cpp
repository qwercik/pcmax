...
int app(int argc, char *argv[]) {
    ...
    cl::Buffer outB(context, CL_MEM_WRITE_ONLY, sizeof(int) * size);

    cl::CommandQueue queue(context, devices[0], 0, &response);
    check(response, "Could not initialze command queue");
    queue.enqueueWriteBuffer(inA, CL_TRUE, 0, sizeof(int) * size, &a[0]);
    queue.enqueueWriteBuffer(inB, CL_TRUE, 0, sizeof(int) * size, &b[0]);
    
    kernel.setArg(0, inA);
    kernel.setArg(1, inB);
    kernel.setArg(2, outB);
    ...
    event.wait();
    check(queue.enqueueReadBuffer(outB, CL_TRUE, 0, sizeof(int) * size, &c[0]), "Could not enqueue read buffer");
